    ...
    <div className="min-h-screen w-full flex flex-col items-center bg-bg">
        ...
        <div className="flex px-5 pt-3 items-end gap-3 self-stretch border-b border-gray-200">
          {TABS.map((tab) => (
            <button
              ...
              className={`flex px-[6px] pb-4 justify-center items-center gap-[10px] border-b-[3px] ${activeTab === tab.key ? "text-primary text-subtitle-01-sb border-primary" : "text-gray-500 text-subtitle-01-m border-transparent"}`}
            >
              {tab.label}
            </button>
          ))}
        ...
        <div className="flex flex-1 w-[375px] flex-col items-center gap-4 mt-5">
          ...
          {filteredBooks?.length === 0 ? (
            ...
            activeTab !== "READING" ? (
              <div className="flex w-[335px] justify-between items-end content-end gap-y-6 flex-wrap">
                {sortedBooks.map((book) => (
                  <BookCard key={book.id} book={book} />
                ))}
              </div>
            ) : (
              <div className="flex w-[335px] items-end content-end gap-y-[32px] justify-between flex-wrap">
                {sortedBooks.map((book) => (
