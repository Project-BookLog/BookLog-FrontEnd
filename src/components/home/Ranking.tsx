import { BOOKS } from "../../data/book.mock";
function Ranking() {
  return (
    <section className="px-5">
      {/* 타이틀 */}
      <div className="mb-3">
        <h2 className="text-title-02 text-[#000000]">실시간 랭킹</h2>
        <p className="text-body-03 text-gray-700">
          현재 북로그에서 인기있는 책이에요.
        </p>
      </div>

      {/* 가로 스크롤 리스트 */}
      <div className="-mx-6 overflow-x-auto no-scrollbar">
        <div className="flex space-x-2 px-5 ">
          {BOOKS.map((book, idx) => {
            const { id, title, author, publisher, coverUrl } = book; 
            return (
              <div
                key={id}
                className={`w-27 flex-shrink-0 ${
                  idx === BOOKS.length - 1 ? "me-10" : ""
                }`}
              >
                {/* 책 이미지 */}
                <div className="w-full h-32.5 rounded-lg overflow-hidden flex items-center justify-center">
                  <img src={coverUrl} className="w-full" />
                </div>

                {/* 책 정보 */}
                <div className="mt-2 space-y-0.5 flex space-around">
                  <div className="w-5">
                    <p className="text-subtitle-02-sb text-black truncate">{id}</p>
                  </div>
                  <div>
                    <p className="text-subtitle-02-sb text-black truncate">{title}</p>
                    <p className="text-caption-02 text-gray-700 truncate">
                      {author}<span className="text-gray-500"> | </span>{publisher}
                    </p>
                  </div>
                </div>
              </div>
            );
          })}
        </div>
      </div>
    </section>
  );
}

export default Ranking;
